...
لتحديد مكان بحث Django عن القوالب الخاصة بنا، تحقق من ملف `settings.py`. في هذا الملف، يجب أن تحتوي المتغير `TEMPLATES` في حقل `DIRS` على المسار إلى قوالبك.
...
**for** يتكرر عبر القائمة، ويمكن الوصول إلى العناصر من خلال المتغير الذي تم إنشاؤه، في هذه الحالة `var`.
